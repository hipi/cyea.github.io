...
module.exports = function(hexo) {
  hexo.extend.helper.register("has_widget", function(type) {
    ...
    return widgets.some(
      widget => widget.hasOwnProperty("type") && widget.type === type
    );
  });
  ...
  hexo.extend.helper.register("get_widgets", function(position) {
    ...
    return widgets.filter(
      widget =>
        widget.hasOwnProperty("position") && widget.position === position
    );
  });
  ...
  hexo.extend.helper.register("column_count", function() {
    ...
    if (
      this.page.__index === true ||
      this.page.__post === true ||
      this.page.__page === true
    ) {
      return 2;
    }
